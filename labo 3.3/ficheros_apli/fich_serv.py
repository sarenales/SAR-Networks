...
PASSWORDS = ("", "sar", "sza") # ""->cualquier password

# este servidor para cntrolar los comandos de los clientes, tiene estados
class State:
...
def session( s ):
	...
	while True:
		message = szasar.recvline( s ).decode( "ascii" ) #recivimos un comando
		if not message:
			return
		...
		if message.startswith( szasar.Command.User ):
			...
			if( user == 0 or PASSWORDS[user] == message[4:] ):
				sendOK( s )
				state = State.Main # recibir cualquier de los comando
			else:
				sendER( s, 3 )
				state = State.Identification
			...
			if state != State.Main:
				sendER( s )
				continue
			filename = os.path.join(FILES_PATH, menssage[4:])
			try:
				filesize = os.path.getsize(filename)
			except:
				sendER(s,5)
...
if __name__ == "__main__":
	...
	while True:
		...
		if( os.fork() ):
			...
			s.close()
			session( dialog ) # a cada hijo se le pasa session para dialogo (arriba)
			dialog.close()
			exit( 0 )